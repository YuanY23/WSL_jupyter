...
class NotebookWidget extends Panel {
  ...
  private _warnInvalidId(badFormat: boolean, duplicateId: boolean, id: string):
    ...
    if (badFormat) {
      options.title = '无效的 nbgrader 单元格 ID';
      options.body = '至少有一个单元格包含无效的 nbgrader ID。单元格 ID 必须至少包含一个字符，并且只能包含字母、数字、连字符和/或下划线。';
      showErrorDialog(options);
      return;
    }
    else if (duplicateId) {
      options.title = '重复的 nbgrader 单元格 ID';
      options.body = `nbgrader ID "${id}" 已被多个单元格使用。请确保所有评分单元格都有唯一的 ID。`;
      showErrorDialog(options);
      return;
    }
  }
  ...
  private _warnSchemaVersion(schemaVersion: number): void {
    ...
    const options = {
      title: '过时的架构版本 (Schema Version)',
      body: bodyWidget,
      buttons: [Dialog.okButton()]
    }
    body.innerText =
      `至少有一个单元格包含旧版本 (${version}) 的 nbgrader 元数据。请备份此笔记本，然后使用以下命令在命令行更新元数据：`;
